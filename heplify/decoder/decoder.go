...
var PacketQueue = make(chan *Packet, 50000)
			...
			if err == nil {
				for _, addr := range addrs {
					...
					if ok && !ipNet.IP.IsLoopback() && ipNet.IP.To4() != nil {
						d.tlsIPs = append(d.tlsIPs, ipNet.IP.To4())
					}
				}
			} else {
				logp.Err("NewDecoder get addrs failure")
			}

		}
	} else {
		logp.Err("NewDecoder get interfaces failure")
	}

	logp.Warn("NewDecoder parse tls ips = %v", d.tlsIPs)
...
func (d *Decoder) checkTransport(srcIP net.IP, srcPort uint16, dstIP net.IP, dstPort uint16, tcp *layers.TCP) (result bool, payLoadList *list.List) {
	...
	//tls port
	if srcPort == 5061 {
		for tlsIP := range d.tlsIPs {
			if srcIP.Equal(d.tlsIPs[tlsIP]) {
				//logp.Warn("checkTransport send tls src_ip = %v, src_port = %d, dst_ip = %v, dst_port = %d, "+
				//	"ack_number = %d, seq_number = %d, next_seq_number = %d, payload = %s",
				//	srcIP, srcPort, dstIP, dstPort, ackNumber, seqNumber, seqNumber+uint32(payloadLen), string(payload))
				payloadList.PushBack(payload)
	...
	if dstPort == 5061 {
		for tlsIP := range d.tlsIPs {
			if dstIP.Equal(d.tlsIPs[tlsIP]) {
				//logp.Warn("checkTransport recv tls src_ip = %v, src_port = %d, dst_ip = %v, dst_port = %d, "+
				//	"ack_number = %d, seq_number = %d, next_seq_number = %d, payload = %s",
				//	srcIP, srcPort, dstIP, dstPort, ackNumber, seqNumber, seqNumber+uint32(payloadLen), string(payload))
				payloadList.PushBack(payload)
				return true, payloadList
