...
import { MesagesData } from '../../detail-dialog/tab-messages/tab-messages.component';
...
@Component({
    selector: 'app-tab-flowls',
    templateUrl: './tab-flow-ls.component.html',
    styleUrls: ['./tab-flow-ls.component.scss'],
    changeDetection: ChangeDetectionStrategy.OnPush,
    encapsulation: ViewEncapsulation.None,
    providers: [{ provide: VIRTUAL_SCROLL_STRATEGY, useClass: CustomVirtualScrollStrategy }]

})
export class TabFlowlsComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    public _isCombineByAlias = false;
    ...
    initData() {
        ...
        const sortedArray: Array<any> = [].concat(
            ...
            ...data.transaction_elements
        );

        this.aliasTitle = Object.keys(hosts).map((i) => {
            let v = hosts[i]
            const __alias = Object.entries(this.dataItem.data.alias).find(j => j[0].startsWith(v));
            const alias = this.dataItem.data.alias[v] || (__alias && __alias[1]);
            ...
            if (regex.exec(v) != null) {
                IP = regex.exec(v)[1]; // gives IP
                PORT = regex.exec(v)[2]; // gives port
            } else {
                // fall back to the old method if things don't work out.
                const al = v.match(/\d+$|(\[.*\]|\d+\.\d+\.\d+\.\d+)/g);
                IP = al[0];
                PORT = al[1] ? ':' + al[1] : '';
            }
            ...
            return {
                ...
                alias: alias && alias.includes(v) ? v : alias,
                IP,
                arrip: [IP.replace(/\[|\]/g, '')],
                PORT
            };
        ...
        // this.labels = data.transaction_elements.map(i => i.call_data.map(q => q.sid )).join(',').split(',').reduce((arr, b) => {
        //     if (arr.indexOf(b) === -1) {
        //         arr.push(b);
        //     }
        //     return arr;
        ...
        const getTxdiffTs = ( dataList = []) => {
            ...
            return (callData[callData.length - 1].micro_ts -  callData[0].micro_ts) / 1000;
        };
        ...
        this.arrayItems = sortedArray.map((vitem, index) => { // vitem
            const { min, max, abs } = Math;

            const transaction = Object.assign({}, vitem);
            ...
            const txSrcPosition = getHostPosition(txSrcIp, txSrcPort, txSrcId),
                  txDstPosition = getHostPosition(txDstIp, txDstPort, txDstId);
            // transaction line
            const txSid = vitem.call_data && vitem.call_data.length ? vitem.call_data[0].sid : '';
            const name = vitem?.name || '' ;
            const txDiff = getTxdiffTs(vitem.call_data);
            const failTxSip = vitem.error_name ? name.split('|').filter(item => item.indexOf(vitem.error_name) > -1).join('|') : ''
            const unfoldTransaction = this.arrayItems.length ? !!this.arrayItems[index].unfoldTransaction : false
            Object.assign(transaction, {
                unfoldTransaction,
                txDiffTs: txDiff.toFixed(3),
                failTxSip: failTxSip,
                txSrcPort,
                txDstPort,
                ...
                txOptions : {
                    ...
                    txRightEnd: this.aliasTitle.length - 1 - max(txSrcPosition, txDstPosition),
                    txIsRedialArrow: txSrcPosition === txDstPosition
                }
            });
            // request response line
            transaction.call_data = transaction.call_data.map((item, key, arr) => {
    ...
    onClickItemShow (index: any) {
        const unfoldTransaction = this.arrayItems[index].unfoldTransaction;
        this.arrayItems[index].unfoldTransaction = !unfoldTransaction;
    }
