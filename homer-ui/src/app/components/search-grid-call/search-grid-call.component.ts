...
export class SearchGridCallComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    public openTransactionDialog(row, mouseEventData = null, callisArray = null) {
        ...
        this._cts.getTransaction(request).toPromise().then(res => {
            const transaction_elements = res.data.transaction_elements || [];
            const allCallIds = transaction_elements.map( i => i.call_data.map(v => v.sid)).join(',').split(',').sort().
            ...
            transaction_elements.forEach(data => data.call_data.forEach(v => timestampArray.push(v.micro_ts)));
            const timestamp = {
