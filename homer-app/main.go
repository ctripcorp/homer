...
func configureAsHTTPServer() {
	...
	e := echo.New()
	e.GET("/vi/health", func(c echo.Context) error {
		responseStatus := &ResponseStatusType{time.Now()}
		response := CheckHealthResponse{"Success", responseStatus}
...
func performV1APIRouting(e *echo.Echo) {
	...
	logrus.Printf("[main] auth jwt = %#v", auth.JwtUserClaim{})
...
func readConfig() {
	...
	config.Setting.SbcIdMap = make(map[string]string)
	config.Setting.OriginSbcIpMap = make(map[string]string)
	config.Setting.OriginVoipRegIpMap = make(map[string]string)
}
...
func makePingKeepAlive(db *gorm.DB, host string, typeData string, node string) {
	...
	for {
		...
		if pingErr != nil {
			...
			logrus.Debug(fmt.Printf("Successful ping: %s, Type: %s, Node: %s \n", host, typeData, node))
		}
