...
func getBody(s *SipMsg) sipParserStateFn {
	...
	if len(s.Body) > 0 {
		xcidStart := strings.Index(s.Body, "XCID=")
		xcidEnd := strings.Index(s.Body, "EXCIDEND")
		if xcidStart != -1 && xcidEnd != -1 && xcidEnd > xcidStart {
			if s.CustomHeader == nil {
				s.CustomHeader = make(map[string]string)
			}
			xcidStart += 5
			xcidEnd += 8
			s.CustomHeader["X-CID"] = s.Body[xcidStart:xcidEnd]
		}
