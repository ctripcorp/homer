...
func (p *Postgres) setup() error {
	cs, err := ConnectString(config.Setting.DBDataTable, config.Setting.DBDataAddr, config.Setting.DBDataUser, config.Setting.DBDataPass)
	if err != nil {
		return err
	}
...
func (p *Postgres) bulkInsert(query string, rows []string) {
	...
	if err != nil || tx == nil {
		logp.Err("%v", err)
		if err != nil && strings.Contains(err.Error(), "mode during recovery") {
			logp.Err("connect is invalid so exit the process")
			os.Exit(1)
		}
		return
	}
