...
func (h *HEP) parse(packet []byte) error {
	...
	if bytes.HasPrefix(packet, []byte{0x48, 0x45, 0x50, 0x33}) {
		...
		if bytes.Equal(packet, []byte{0x54, 0x44, 0x4c, 0x42}) ||
			bytes.Equal(packet, []byte{0x41, 0x31, 0x30, 0x0}) {
			return errors.New("udp health check")
		}

		err = h.Unmarshal(packet)
		if err != nil {
			logp.Warn("malformed packet with length %d and value = %#v which is neither hep nor protobuf encapsulated", len(packet), packet)
			return err
		}
	}
